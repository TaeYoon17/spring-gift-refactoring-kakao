---
name: API 관리자 스킬
description: 프로젝트의 컨트롤러, DTO, 엔티티 파일들을 분석하여 `./api_receipt/` 디렉토리에 멋진 HTML API 명세서를 자동 생성하고 업데이트하는 스킬. SWAGGER나 Spring REST Docs 같은 외부 라이브러리 없이, 프로젝트 구조와 어노테이션을 직접 분석하여 명세서를 생성합니다. 생성된 명세서는 한국어로 작성되고, 다크 모드 기반의 모던한 디자인으로 제공됩니다.
---
